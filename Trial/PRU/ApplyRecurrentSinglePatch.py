...
if __name__ == "__main__":
    ...
    upInitDisp, initDisp, initDisp1, initDispRe0 = estimate_initial_disparity( img0, img1, recCom, (initDispH, initDispW) )
    ...
    outFnBase = "%s/initDisp1" % (args.outdir)
    save_disp(outFnBase, initDisp1, dispMin=0, dispMax=192)

    outFnBase = "%s/initDispRe0" % (args.outdir)
    save_disp(outFnBase, initDispRe0, dispMin=0, dispMax=192)
    ...
    dispRes1CPU = convert_cuda_tensor_2_image(extra[2])
    upDisp1CPU  = convert_cuda_tensor_2_image(extra[3])
    ...
    save_disp(outFnBase, dispRes1CPU * 10, dispMin=-192, dispMax=192)

    # Save the upDisp1.
    outFnBase = "%s/DispPatch_%d_%d_%d_%d_UpDisp1" % (args.outdir, coor[0], coor[1], coor[2], coor[3])
    save_disp(outFnBase, upDisp1CPU, dispMin=0, dispMax=192)

    # Save the refinement disparity.
    outFnBase = "%s/DispPatch_%d_%d_%d_%d_dispRe0" % (args.outdir, coor[0], coor[1], coor[2], coor[3])
    save_disp(outFnBase, dispRe0CPU * 10, dispMin=-192, dispMax=192)
