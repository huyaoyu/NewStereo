...
class RecurrentBlock(nn.Module):
    ...
    def one_shot(self, f0, f1, upsampledScaledDisp):
        f0 = self.fe(f0)
        f1 = self.fe(f1)
...
class RecurrentStereo(nn.Module):
    ...
    def one_shot(self, stack0, stack1, initDisp):
        """
        stack0 and stack1 are the warped, stacked features/images.
        ...
        disp0, _ = self.rb0.one_shot( f00, f01, initDisp )

        # ========== Disparity refinement. ==========
        dispRe0 = self.refine( torch.cat((f00, disp0), 1) )
        disp0 = disp0 + dispRe0

        return disp0, ( dispRe0 )
