...
class Deconv_DoubleSize(nn.Module):
    def __init__(self, inCh, outCh, k=3, op=0, activation=None):
        super(Deconv_DoubleSize, self).__init__()

        if ( not is_odd(k) ):
            raise Exception("k must be an odd number. k = {}. ".format(k))

        moduleList = [ nn.ConvTranspose2d(inCh, outCh, kernel_size=k, stride=2, padding=k//2, dilation=1, output_padding=op, bias=True) ]
