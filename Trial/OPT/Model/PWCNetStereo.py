...
class ConvExtractor(nn.Module):
    def __init__(self, inCh, outCh, lastActivation=None, bias=False):
        super(ConvExtractor, self).__init__()

        moduleList = [ \
            cm.Conv_Half( inCh, outCh, activation=nn.LeakyReLU(0.1), bias=bias ), \
            cm.Conv_W( outCh, outCh, activation=nn.LeakyReLU(0.1), bias=bias ), \
            cm.Conv_W( outCh, outCh, bias=bias ) ]
...
class CostRegulator(nn.Module):
    def __init__(self, inCh, interChList, lastActivation=None):
        ...
        self.regulators.append( cm.Conv_W( inCh + cumCh[n-1], interChList[n-1], bias=True ) )
...
class EDRegression(nn.Module):
    def __init__(self, inCh):
        ...
        self.rg1 = cm.Conv_W( 64, 1, k=3, activation=None, bias=True )
...
class PWCNetStereoRes(nn.Module):
    def __init__(self, params):
        ...
        self.upCost3Normalization = FeatureNormalization(nd + nd)
        self.upCost2Normalization = FeatureNormalization(nd + nd)
        ...
        self.disp2 = Cost2DisparityAndFeatureRes((nd + nd) + (nd + nd), interChList)
        self.disp1 = Cost2DisparityAndFeatureRes((nd + nd) + (nd + nd), interChList, flagUp=True)
    ...
    def forward(self, gray0, gray1, grad0, grad1):
        ...
        upCost3 = self.upCost3Normalization( 
            F.interpolate(cost3, (height2, width2), mode="bilinear", align_corners=False ) )
        cost2 = torch.cat( (cost2, f20, upCost3), 1 )
        ...
        upCost2 = self.upCost2Normalization( 
            F.interpolate(cost2, (height1, width1), mode="bilinear", align_corners=False) )
        cost1 = torch.cat( (cost1, f10, upCost2), 1 )
        ...
        disp0 = upDisp1 * F.relu( 1.0 + dispRe0, inplace=True )
