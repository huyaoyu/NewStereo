...
class Conv_W(nn.Module):
    def __init__(self, inCh, outCh, k=3, activation=None, bias=False):
        """
        ...
        moduleList = [ \
            ...
            nn.Conv2d(inCh, outCh, kernel_size=k, stride=1, padding=0, dilation=1, bias=bias)
        ]
...
class Conv_Half(nn.Module):
    def __init__(self, inCh, outCh, k=3, activation=None, bias=False):
        ...
        moduleList = [ \
            nn.ReflectionPad2d(padding=k//2), 
            nn.Conv2d(inCh, outCh, kernel_size=k, stride=2, padding=0, dilation=1, bias=bias)
        ]
